...
Long delimiter runs, the same day. A paragraph of 12,000 asterisks a side
overflowed the stack in Lezer's emphasis tree writer, a frame per level of
nesting, and the editor never built its view. `runs.js` in
`vendor-src/src/markdown/` takes a run of twenty or more `*` or `_` as text
ahead of Emphasis, with no node of its own: no reader that matters gives
such a run a meaning (Pandoc's markdown leaves `******foo******` literal).
Two mutations, both caught: **LR1** `mdmLongRuns` left out of the extension
set in `index.js` -> *a run of emphasis delimiters at the long-run length or
more is text* and *a paragraph of 12,000 asterisks a side parses, and
quickly* (the `RangeError` of the overflow itself), with *a run one short of
the long-run length still nests* green either way; **LR2** the bundle of the
commit before put back under the editor -> *a paragraph of twelve thousand
asterisks a side opens, as text* in `webview-editing.test.js`, which waited
its 20 s for a first line that never came.
...
The look-aheads that read the rest of the input (P7, G038), 2026-09-18: a
callout's opener looked for its closer by reading raw lines to the end of the
input, once per opener, so a document of openers that never close cost the
square of its length: 3935 ms for 5000 of them in Node, against the 3 s
CodeMirror gives a background parse, past which the tree is never committed
and the rest of the document stays raw source. The closers are now read off
the whole input once per parse, with a stack (a closer answers the nearest
opener waiting), as `links.js` reads its definitions: 24 to 55 ms for the
same 5000. The raw TeX block of P6-b had the same look-ahead from every
`\begin{env}` and is given the same cure, the last `\end{env}` of each name
kept per parse. Both are pinned by what the extensions read through
`input.read`, which the parser itself does not use: at most four times the
length of the input, where 3000 openers read it 3000 times. The editor-side
half of the verdict, the parse worker's budget, is CodeMirror's and is not
touched; no test in Chrome times the parse, since a bound tight enough to
fail on the old code would be a flake on a loaded machine. Six mutations,
all caught, the bundle not rebuilt for them (the vendor tests read the
sources): **CO1** the callouts' scan run again at every opener -> the
vendor's *callout: the closers are looked for once per parse*; **CO2** a
closer given to the first opener waiting and not the nearest -> the same and
*callout: unterminated*; **CO3** a closer with no opener waiting counted ->
the first; **RX1** the environments' scan run again at every opener -> the
vendor's *raw TeX: the end of an environment is looked for once per parse*;
**RX2** an end above the opener taken for its closer -> the same; **RX3**
the first end of a name kept and not the last -> the same, once the case of
two environments of one name was added (not caught before it).
...
The cost of a caret move and of a keystroke (P7, the scale findings of the
bench), the same day. Every transaction that moved a caret, changed the text
or landed a piece of the tree rebuilt the decorations of the whole document:
on the bench's 22 805-line file, 95 ms (57 of the walk, 12 of the loop over
the lines, 21 of sorting the set, profiled), so an arrow key cost 106 ms and
a letter about as much. The decorations of a top-level block depend on its
own text and nodes, on the carets that touch it, on the count of lines the
host keeps back and on the link definitions, and on nothing else, so the set
that stood is kept, moved along with the changes, and only the blocks are
rebuilt that a caret touched or touches now, that a change fell in, or over
which the top-level blocks of the two trees differ (compared from both ends,
the ones after the last change by their places moved along). The whole is
rebuilt when the number of lines changes (a drawn rule, table and empty card
carry their line's number), when the host keeps another count back, when a
link definition is in what changed or the language is taken up again with
another set of them, and when the stretch is more than half the document.
The link definitions are read once per tree and carried to the next tree
when nothing that holds one changed. After it, on the same file: 2 ms in the
field (median), 23 ms for an arrow key and 26 for a letter all told; a line
break still rebuilds the whole, 150 ms there. What is left of the 23 is
CodeMirror's: it compares the old and the new sets range by range wherever a
chunk of them holds a replaced range, which is nearly every chunk of a
Markdown document (9 ms, profiled), and the browser's own work. Found by the
stress that wrote these tests, on that same file: the marker of an item whose
text starts on the next line takes the line break with it and ends where the
next line begins, and a rebuild of that line dropped it, going by where a
decoration ends; a decoration is now its block's by where it starts. The
random walk was run outside the suite too, 3800 steps over the bench's
document, its huge and pathological variants and `example.mdm`, with no
difference. Twelve mutations of the rebuild, eleven caught: **PR1** a
decoration dropped by where it ends -> *the marker of an item whose text
starts on the next line*; **PR2** the blocks a caret left not rebuilt -> four
of the tests; **PR3** the changed stretch left out -> *a change made away
from the caret*; **PR4** the difference of the trees left out -> five of
them; **PR5** the count of lines not looked at -> *what reaches past its own
block*; **PR6** a changed definition taken in part -> the same; **PR7** the
language taken up again taken in part -> *the language taken up again*, once
that test had a document short enough to be parsed at one go (in a long one
the parse lands in pieces, each rebuilt as it comes, and the guard was not
needed to pass); **PR8** a region not grown to whole blocks -> three of them;
**PR9** the count the host keeps back not looked at -> *what reaches past its
own block*, once its message came before anything was typed (after, the
merge of the message with the edit still in flight put a line in, and that
rebuilt the whole by itself); **PR10** the whole rebuilt every time -> the
counts, in four of them; **PR11** the stretch of the old tree's blocks left
out -> not caught: in every case I could build it lies inside the changed
stretch or the new tree's blocks, and it stays as the cheaper side of a
doubt; **PR12** the blocks a caret enters not rebuilt -> two of them. The two
things that grew with use, six mutations, all caught: **KX1** nothing ever
dropped from the equations kept -> *the equations kept rendered are bounded*;
**KX2** one asked for again not made the newest -> the same, once it looked
at the render itself and not at the key, which a render made again after it
went satisfies; **KX3** the count not brought down with what goes -> the
same; **OL1** the outline painted at a caret move -> *a caret move marks
another row of the outline*; **OL2** painted at every keystroke -> the same;
**OL3** the rows' places not moved with the text -> the same (the click
between the typing and the painting).
